    ...
    if (cmdOPS == 'CREATE') {
        ...
        if (index >= 0) {
            ...
            fs.writeFileSync(`pipeline.sh`, [
                ...
                `if [ $? -eq 0 ]; then`,
                `   if [ "$#" -eq 2 ]; then`,
                `       docker-compose -f docker-compose.yml --project-name simplify-pipeline build --force-rm --no-cache;`,
                `       docker-compose -f docker-compose.yml -f docker-compose.$1.yml --project-name simplify-pipeline build $2;`,
                `   else`,
                `       echo "Missing argument: require stage argument to run - (ex bash pipeline.sh build [service])";`,
                `   fi`,
                `fi`
            ].join('\n'))
        }
